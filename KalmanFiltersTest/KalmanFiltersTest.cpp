...
VectorXd xObjectState;	// object state
MatrixXd PObjectCovariance;	// object covariance matrix
VectorXd uExternalMotion;	// external motion
MatrixXd FStatetransition; // state transition matrix
MatrixXd HMeassurement;	// measurement matrix
MatrixXd RMeassurementCovariance;	// measurement covariance matrix
MatrixXd Identity; // Identity matrix
MatrixXd QProcessCovariance;	// process covariance matrix
...
void filter(VectorXd &x, MatrixXd &P);
	...
	int main() {
		...
		*/
		...
		xObjectState = VectorXd(2);
		xObjectState << 0, 0;

		PObjectCovariance = MatrixXd(2, 2);
		PObjectCovariance << 1000, 0, 0, 1000;

		uExternalMotion = VectorXd(2);
		uExternalMotion << 0, 0;

		FStatetransition = MatrixXd(2, 2);
		FStatetransition << 1, 1, 0, 1;

		HMeassurement = MatrixXd(1, 2);
		HMeassurement << 1, 0;

		RMeassurementCovariance = MatrixXd(1, 1);
		RMeassurementCovariance << 1;

		Identity = MatrixXd::Identity(2, 2);

		QProcessCovariance = MatrixXd(2, 2);
		QProcessCovariance << 0, 0, 0, 0;
	...
	void filter(VectorXd &xObjectState, MatrixXd &PObjectCovariance) {
		...
		for (unsigned int n = 0; n < measurements.size(); ++n) {
			...
			VectorXd zMeassurement = measurements[n];
			//YOUR CODE HERE
